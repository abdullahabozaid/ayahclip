...
| Native iOS beta | Shared-editor foundations implemented; host approval pending | 70 unit and 8 UI tests pass. The local branch removes fabricated starter verses from creator drafts, adds still/video/audio import, Share Extension image support, selectable Quran ranges, shared recognition routing, versioned web/native project persistence, opaque private-media streaming, a cancellation-safe system-picker coordinator, a single lifecycle-safe shared-editor host session, purpose-safe in-Studio B-roll import and chunked MP4 return to Photos. The approved SwiftUI presentation and physical-device journeys remain release gates. |
| TestFlight distribution | Internal prototype only | App Store Connect record `6792317617`, bundle `app.ayahclip.mobile`, version `0.1.0` build `1` exists. Build 1 predates the current parity work and must not be promoted externally; the replacement archive requires the approved host, identity verification and physical-device smoke tests. |
...
| Future accounts, admin, CRM and growth | Planned | `docs/future-platform-and-growth-plan.md` defines adoption triggers, an RLS/private-storage architecture, tenant-isolation and deletion contracts, operator roles and auditing, consent-safe CRM topics, operational feature controls, AI boundaries, cost controls, and a four-week permission-based beta promotion cadence. None of these future systems are presented as shipped. |
