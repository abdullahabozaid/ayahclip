...
- The verified public deployment returns `nosniff`, `DENY` framing, strict referrer policy, a restrictive feature policy, HSTS, and a CSP baseline that denies framing, object embedding, and hostile base URLs. Re-run this gate after the VPS cutover.
...
- Telemetry accepts only event-specific fields from the versioned schema in `src/lib/telemetry-schema.ts`, caps payloads at 2 KB before consuming rate-limit capacity, rejects cross-site browser requests, and does not log source media, file names, Quran text, transcripts, URLs, raw errors, user agents, referrers, or client IP addresses. Route tests prove deterministic 600-accepted/1-throttled behavior under a 601-request same-client burst; the ceiling accommodates unrelated creators behind a shared school, mosque, office, or carrier address while still bounding accidental loops.
...
- `npm run test:local-load` starts the built production server on a private port and provides a repeatable warm-instance load gate without touching production or paid providers. The current release build completed 200 concurrent public-page requests at 1121.7 ms p95, accepted exactly 600 of a 720-request same-client telemetry burst before returning 120 `429` responses with `Retry-After`, accepted 200 events across ten independent client buckets, and rejected 60 cross-site caption/checkout attempts before provider access. This does not prove VPS reverse-proxy or firewall behaviour.
...
1. Warm-instance throttling is defense-in-depth, not a distributed guarantee. Before broad paid-API traffic, configure fixed-window limits for `/api/social-caption`, `/api/pexels`, `/api/support/checkout`, and `/api/telemetry` at the trusted reverse-proxy or upstream firewall, keyed by the verified client IP. Start in log-only mode, confirm ordinary creator traffic, then enforce.
...
3. Product events currently live in bounded Docker runtime logs. A real analytics/CRM backend needs a documented retention period, access roles, deletion policy, schema migration process, and incident-response owner before ingesting user-linked records.
...
5. Rotate all production secrets on an incident or staff-access change and review VPS, SSH, and environment-file access before each public release.
