...
- Public deployments return 404 for the disk-backed clip library routes. `/api/save-export` accepts only localhost or private-LAN browser origins, validates names/extensions, and caps files at 500 MB.
...
- Telemetry accepts only the fixed schema in `src/lib/telemetry-schema.ts`, caps payloads at 2 KB, rejects cross-site browser requests, and does not log source media, file names, Quran text, transcripts, URLs, raw errors, user agents, referrers, or client IP addresses.
...
- Checkout validates the exact same origin, normalizes the donation amount, constructs return URLs from the deployment origin, and keeps the Stripe key server-only.
