...
make_plist() {
  ...
  plutil -insert CFBundleVersion -string '4' "$path"
}
...
"$verifier" "$archive" 'app.ayahclip.mobile' 'AyahClip' '0.1.0' '4' >/dev/null

if "$verifier" "$archive" 'app.ayahclip.mobile' 'AyahClip' '0.1.0' '5' >/dev/null 2>&1; then
